...
# Overview
TravelWise is an iOS app that helps travellers gather information regarding flights and accommodations available around the globe. It is built using the **Swift** programming language and its **UIKit** framework. **Two APIs** are used for fetching **real time legitimate data**. More about API usage [here](#api-usage). This app was made in collaboration with Bogdan Fomin, a Computer Science student from Iasi, Romania. I worked on the [Accommodations Section](#accommodations-section), while he was tasked with the [Flights Section](#flights-section).
